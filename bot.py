...
def catch_pokemon(client, message):
    ...
    if pokemon_name.lower() == announced_pokemon["name"].lower():
        ...
        if catch_probability <= announced_pokemon["catch_rate"]:
            client.send_message(chat_id=message.chat.id, text="Congratulations (message.from_user.username) ! You caught {}!".format(announced_pokemon["name"], reply_to_message_id=message.message_id))
