...
def view_pokedex(client, message):
    ...
    if pokedex_data:
        ...
        client.send_message(message.chat.id, "** {} Pokedex:**\n{}\n**Total Pokémon Caught:** {}".format(message.from_user.username, pokedex_list, pokemon_count), reply_to_message_id=message.message_id)
    else:
...
def catch_pokemon(client, message):
    ...
    if pokemon_name.lower() == announced_pokemon["name"].lower():
        ...
        if catch_probability <= announced_pokemon["catch_rate"]:
            client.send_message(chat_id=message.chat.id, text="Congratulations {}! You caught {}!".format(message.from_user.username, announced_pokemon["name"], reply_to_message_id=message.message_id))
